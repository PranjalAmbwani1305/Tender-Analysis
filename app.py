...
# ══════════════════════════════════════════════════════════════════════════════
# CONFIG
# Keys are read ONLY from Streamlit Cloud Secrets.
# Go to:  your app → ⋮ menu → Settings → Secrets  and paste:
#
#   HUGGINGFACE_API_KEY = "hf_xxxx"
#   PINECONE_API_KEY    = "xxxx-xxxx-xxxx"
#   PINECONE_INDEX      = "tender"
#   PINECONE_ENV        = "us-east-1"
#   PINECONE_CLOUD      = "aws"
#
# Nothing is stored in this file, in GitHub, or in any zip.
# ══════════════════════════════════════════════════════════════════════════════
...
def get_pinecone_index():
    if not PINECONE_API_KEY:
        return None, "PINECONE_API_KEY not set — add it in Streamlit Cloud → Settings → Secrets."
    try:
        ...
        st.session_state["pinecone_dim"] = stats.get("dimension", 768)
        return index, None
    except Exception as e:
        return None, str(e)
...
def _embed(text):
    dim = st.session_state.get("pinecone_dim", 768)
    vec = [0.0] * dim
        ...
        for section_name, section_text in sections.items():
            for j, chunk in enumerate(_chunk(section_text)):
                vec_id = (
                    ...
                    if j == 0
                    else f"{filename}_{section_name}_{j}"
                )
                vectors.append({
                    "id":     vec_id,
                    "values": _embed(chunk),
...
# ══════════════════════════════════════════════════════════════════════════════
# NER — Regex-based Named Entity Recognition
# ══════════════════════════════════════════════════════════════════════════════
PATTERNS = {
    "tender_number": [
    ...
    "location":        [
        r"(?:Location|Place|Site|Project\s*Location)\s*[:\-]?\s*([A-Za-z\s,]+?(?:District|City|Town|Maharashtra|Gujarat|Delhi|Karnataka))",
        r"(?:District|Dist\.?)\s*[:\-]?\s*([A-Za-z\s]+?)(?:\n|,)",
    ],
    "authority":       [
        r"(?:Issued?\s*by|Issuing\s*Authority|Department|Ministry)\s*[:\-]?\s*([A-Za-z\s,&]+?)(?:\n|\.|,)",
...
ELIGIBILITY_TRIGGERS = [
    "eligible", "eligibility", "qualification", "criteria", "experience",
    "turnover", "registered", "empanelled", "class", "category",
    "valid license", "ISO", "net worth", "solvency", "minimum requirement",
    "shall have", "must have", "should have",
]
...
def _extract_scope(text):
    m = re.search(
        r"(?:Scope\s*of\s*Work|Description\s*of\s*Work|Nature\s*of\s*Work)\s*[:\-]?\s*([\s\S]{20,400}?)(?:\n\n|\d\.|[A-Z]{3,}:)",
        text, re.IGNORECASE,
    )
    if m:
    ...
    if m2:
        return re.sub(r"\s+", " ", m2.group(1)).strip()[:300]
    return None
...
def extract_entities(raw_text):
    ...
    base = {
        k: _match_first(text, PATTERNS[k])
        for k in [
            "tender_number", "deadline", "emd", "estimated_value", "duration",
            "location", "authority", "contact_email", "contact_phone",
            "bid_opening", "prebid_meeting",
        ]
    }
    base["eligibility"] = _extract_eligibility(raw_text)
    base["scope"]       = _extract_scope(raw_text)
    return base
...
def extract_text_from_image(file_bytes):
    try:
        ...
        return "OCR unavailable — add tesseract-ocr to packages.txt.", 1
    except Exception as e:
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["📤 Upload Tender", "📋 All Tenders", "🔍 RAG Search", "📊 Report", "🔐 Security"],
...
if page == "📤 Upload Tender":
    ...
    if not HUGGINGFACE_API_KEY:
        st.warning("⚠️ HUGGINGFACE_API_KEY missing — add it in Streamlit Cloud → your app → ⋮ → Settings → Secrets.")
    if not pc_ok:
        st.warning(f"⚠️ Pinecone not connected — tenders stored in session only. {_pc_err or ''}")

    uploaded_files = st.file_uploader(
        "Drop tender files here",
            ...
            for uf in uploaded_files:
                with st.expander(f"Processing: {uf.name}", expanded=True):
                    ...
                    if uf.type == "application/pdf":
                        ...
                        raw_text, pages = extract_text_from_image(file_bytes)
                    ...
                    if pc_msg:
                        st.caption(f"Pinecone: {pc_msg}")

                    fields = {
                        "Tender Number": entities.get("tender_number"),
                        "Deadline":      entities.get("deadline"),
                        "EMD":           f"Rs {entities['emd']}" if entities.get("emd") else None,
                        "Est. Value":    entities.get("estimated_value"),
                        "Authority":     entities.get("authority"),
                        "Location":      entities.get("location"),
                        "Duration":      entities.get("duration"),
                        "Bid Opening":   entities.get("bid_opening"),
                        "Contact":       entities.get("contact_email"),
                    }
    ...
    with st.expander("ℹ️ What gets extracted", expanded=not uploaded_files):
        ...
        for i, f in enumerate([
            "Tender number", "Deadline", "EMD", "Est. value", "Eligibility",
            "Scope", "Authority", "Contact", "Location", "Bid opening",
            "Pre-bid date", "Duration",
        ]):
        ...
        for tender in st.session_state.tenders:
            ...
            with st.expander(f"{badge} {tender['filename']}", expanded=False):
                ...
                with tabs[0]:
                    c1, c2 = st.columns(2)
                    for k, v in {
                        "Tender Number": e.get("tender_number"),
                        "Authority":     e.get("authority"),
                        "Location":      e.get("location"),
                        "Deadline":      e.get("deadline"),
                        "Bid Opening":   e.get("bid_opening"),
                        "Pre-Bid":       e.get("prebid_meeting"),
                    }.items():
                        if v: c1.markdown(f"**{k}:** `{v}`")
                    for k, v in {
                        "EMD":       f"Rs {e['emd']}" if e.get("emd") else None,
                        "Est. Value": e.get("estimated_value"),
                    ...
                    if e.get("scope"):
                        st.markdown(f"**Scope:** {e['scope']}")
                    if e.get("eligibility"):
                        st.markdown("**Eligibility:**")
                        for item in e["eligibility"]:
                            st.markdown(f"  ✓ {item}")
                ...
                with tabs[1]:
                        ...
                        for i, s in enumerate([
                            ...
                            if scols[i % 2].button(s, key=f"sug_{tid}_{i}"):
                                with st.spinner("Thinking…"):
                                    answer = ask_hf(s, tender["raw_text"])
                                history += [
                                    {"role": "user",      "content": s},
                    ...
                    if user_q:
                        ...
                        with st.spinner("Generating answer…"):
                            answer = ask_hf(user_q, tender["raw_text"])
                        history.append({"role": "assistant", "content": answer})
                        st.session_state.chat_history[tid] = history
                        st.rerun()

                with tabs[2]:
                    preview = tender["raw_text"][:3000]
                    if len(tender["raw_text"]) > 3000:
                        preview += "…"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        query = st.text_input("Search query", placeholder="e.g. EMD amount, eligibility, scope…")
    with c2:
        top_k = st.selectbox("Top K", [3, 5, 10], index=0)
    ...
    if st.button("🔍 Search", type="primary") and query:
        ...
        if search_err:
            ...
            for match in matches:
                ...
                section_html = (
                    f"&nbsp;·&nbsp;<span style='color:#10b981;font-size:12px'>{section}</span>"
                    if section else ""
                )
                st.markdown(
                    f"""<div style="background:#1a1d2e;border:1px solid #2a2d3e;
                    border-left:3px solid #f59e0b;border-radius:8px;padding:14px;margin-bottom:10px;">
                    <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                    <span><strong style="color:#818cf8">{fname}</strong>{section_html}</span>
                    <span style="color:#f59e0b;font-family:monospace">{score:.2%}</span></div>
                    <div style="color:#c8c8d8;font-size:13px;line-height:1.6">{meta.get('text','')[:400]}</div>
                    </div>""",
                    unsafe_allow_html=True,
                )
            st.markdown("---")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    rows = [
        {
            "File":      t["filename"],
            "Authority": t["entities"].get("authority")       or "—",
            "Deadline":  t["entities"].get("deadline")        or "—",
            "EMD":       f"Rs {t['entities']['emd']}" if t["entities"].get("emd") else "—",
            "Value":     t["entities"].get("estimated_value") or "—",
            "Location":  t["entities"].get("location")        or "—",
            "Pinecone":  "✅" if t.get("pinecone_indexed") else "—",
        }
        for t in st.session_state.tenders
    ]
    ...
    if st.button("🤖 Generate AI Report", type="primary"):
        summaries = "\n\n".join([
            f"Tender {i+1}: {t['filename']}\n"
            f"  Authority: {t['entities'].get('authority','N/A')}\n"
            f"  EMD: Rs {t['entities'].get('emd','N/A')}\n"
            f"  Value: {t['entities'].get('estimated_value','N/A')}\n"
            f"  Deadline: {t['entities'].get('deadline','N/A')}\n"
            f"  Location: {t['entities'].get('location','N/A')}"
            for i, t in enumerate(st.session_state.tenders)
        ])
        prompt = (
            f"Analyze these {len(st.session_state.tenders)} Indian government tenders:\n"
            "1. Total portfolio value\n2. Upcoming deadlines\n"
            "3. Highest value opportunity\n4. Common eligibility themes\n"
            "5. 3 strategic recommendations\n\nDATA:\n" + summaries
        )
        with st.spinner("Generating report…"):
            report = ask_hf(prompt, summaries)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    LAYERS = [
        ...
        ("L2", "Authentication",     "API keys stored ONLY in Streamlit Cloud Secrets — never in code or GitHub"),
        ...
        ("L5", "Rate Limiting",      "HF Inference API rate limits. st.cache_resource prevents Pinecone reconnect storms"),
    ...
    if st.button("🔍 Run Security Scan", type="primary"):
        ...
        for i, (num, name, desc) in enumerate(LAYERS):
            ...
            if   num == "L1": results[num] = ("pass", "File validation active")
                ...
                ok = bool(HUGGINGFACE_API_KEY and PINECONE_API_KEY)
                results[num] = (
                    "pass" if ok else "warn",
                    "Keys from Streamlit Secrets ✓" if ok else "One or more keys missing",
                )
            elif num == "L3": results[num] = ("pass", "Single-user mode")
            elif num == "L4": results[num] = ("pass", "HTTPS + AES-256")
            elif num == "L5": results[num] = ("pass", "cache_resource + HF limits")
            elif num == "L6": results[num] = ("pass", "Streamlit sandbox")
            elif num == "L7":
                events = sum(len(v) for v in st.session_state.chat_history.values())
                results[num] = ("pass", f"{events} events logged")
        prog.empty()

        passed = sum(1 for r in results.values() if r[0] == "pass")
        (st.success if passed == len(LAYERS) else st.warning)(
            f"{'✅' if passed == len(LAYERS) else '⚠️'} {passed}/{len(LAYERS)} layers passed"
        )
        for num, name, desc in LAYERS:
            ...
            st.markdown(
                f"""<div style="background:#1a1d2e;border:1px solid {color};border-radius:8px;
                ...
                <div style="flex:1">
                  <strong style="color:#e8e8f0">{name}</strong>
                  <div style="color:#6b6b88;font-size:12px;margin-top:2px">{desc}</div>
                </div>
                <span style="font-size:18px">{icon}</span>
                <span style="color:#9999b0;font-size:11px;font-family:monospace">{detail}</span>
                </div>""",
                unsafe_allow_html=True,
            )
    else:
